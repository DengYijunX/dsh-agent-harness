...
阶段二已完成：DSH Service/Fiber 装配、核心依赖固定、Loader/Schema、Include/YAML、CLI、可安装 tarball、Tool Registry/Permission Policy、Concurrency Gate、Approval/Sandbox、受控写文件和 Shell、可配置危险工具、审批记忆、JSONL 审批、审批 Scope、真实沙箱测试、结构化审计事件、Context Projection、Session Resume、真实 DeepSeek 主链路、真实工具治理链路、真实 DSH 装配链路和测试分类均已具备；下一步补齐 token 级预算、摘要生成和损坏事件恢复，并在前端阶段加入 Chrome CDP。
...
| 2026-09-04 | 增加真实工具治理实测 | 用真实 DeepSeek 触发 write_file，验证权限拒绝发生在文件系统写入之前，并修正测试分类入口 |
| 2026-09-07 | 增加真实 DSH 装配测试 | 用真实 DeepSeek 通过 createHarnessContext 验证 Loader/Context/Fiber、工具、Session 和 Runtime 的连接 |
